...
echo "<table>\n";
echo "<tr><td>devid</td><td>设备id</td><td>".$st["devid"]."</td></tr>";
echo "<tr><td>temperature</td><td>温度</td><td>".$st["temperature"]."°C（尚不明确报告值和实际值的关系）</td></tr>";
echo "<tr><td>humidity</td><td>湿度</td><td>".$st["humidity"]."%（尚不明确报告值和实际值的关系）</td></tr>";
echo "<tr><td>run_favor</td><td>运行偏好</td><td>". favorstatus($st["run_favor"])."</td></tr>";
echo "<tr><td>powerlight</td><td>电源指示灯</td><td>".$st["powerlight"]."</td></tr>";
...
echo "<tr><td>auto_sleep</td><td>自动进入睡眠模式</td><td>".$st["auto_sleep"]."</td></tr>";
echo "<tr><td>prompt_tone</td><td>操作提示音</td><td>".$st["prompt_tone"]."</td></tr>";
echo "<tr><td>power_button_mode</td><td>电源键操作模式</td><td>".powermode($st["power_button_mode"])."</td></tr>";
echo "<tr><td>uv_status</td><td>紫外线灯状态</td><td>".$st["uv_status"]."</td></tr>";
echo "<tr><td>lock_status</td><td>按键锁定状态</td><td>".$st["lock_status"]."</td></tr>";
